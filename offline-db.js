(() => {
  ...
  async function deleteSubmission(id){const db=await openDB();return new Promise((resolve,reject)=>{const tx=db.transaction("submissions","readwrite");tx.objectStore("submissions").delete(id);tx.oncomplete=()=>resolve();tx.onerror=()=>reject(tx.error);});}
  ...
  window.AAOfflineDB={openDB,putSubmission,getSubmission,getAllSubmissions,deleteSubmission,putMedia,getMedia,deleteMedia,setSetting,getSetting};
})();